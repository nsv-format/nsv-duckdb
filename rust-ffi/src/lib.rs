...
//!
//! Both store pre-decoded `Vec<Vec<String>>`. Pointers from cell accessors
//! are stable until the handle is freed.

use std::ffi::CString;
use std::os::raw::c_char;

// ── Full decode (bind-time) ─────────────────────────────────────────
...
pub extern "C" fn nsv_col_count(handle: *const NsvHandle, row: usize) -> usize {
    if handle.is_null() { return 0; }
    unsafe { &*handle }.data.get(row).map_or(0, |r| r.len())
}

#[no_mangle]
pub extern "C" fn nsv_cell(
    handle: *const NsvHandle, row: usize, col: usize, out_len: *mut usize,
) -> *const c_char {
...
pub extern "C" fn nsv_encoder_new() -> *mut NsvEncoder {
    Box::into_raw(Box::new(NsvEncoder { rows: Vec::new(), current_row: Vec::new() }))
}

#[no_mangle]
pub extern "C" fn nsv_encoder_push_cell(enc: *mut NsvEncoder, ptr: *const u8, len: usize) {
    if enc.is_null() || ptr.is_null() { return; }
...
pub extern "C" fn nsv_encoder_push_null(enc: *mut NsvEncoder) {
    ...
    unsafe { &mut *enc }.current_row.push(String::new());
}
...
#[no_mangle]
pub extern "C" fn nsv_encoder_finish(enc: *mut NsvEncoder, out_ptr: *mut *mut u8, out_len: *mut usize) {
    if enc.is_null() { return; }
    ...
    let bytes = nsv::encode(&e.rows).into_bytes();
    let len = bytes.len();
...
mod tests {
    ...
    fn test_decode_roundtrip() {
        ...
        let mut len = 0usize;
        let cell = nsv_cell(handle, 1, 0, &mut len);
