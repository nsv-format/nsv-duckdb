...
static unique_ptr<GlobalTableFunctionState> NSVInit(ClientContext &ctx, TableFunctionInitInput &input) {
	...
	if (!bind.raw_buffer.empty() && !state->column_ids.empty() && state->column_ids.size() < ncols) {
...
static Value ReadAndCastCell(ClientContext &ctx, const NSVBindData &bind, const NSVScanState &state, idx_t row_idx,
                             idx_t scan_col) {
	...
	while (emitted < STANDARD_VECTOR_SIZE && state.current_row < total_rows) {
		...
		if (!has_filters) {
			for (idx_t out_col = 0; out_col < output_col_count; out_col++) {
				...
				idx_t scan_col = state.projection_ids.empty() ? out_col : state.projection_ids[out_col];
				output.data[out_col].SetValue(emitted, ReadAndCastCell(ctx, bind, state, row_idx, scan_col));
			}
			emitted++;
			continue;
		}
