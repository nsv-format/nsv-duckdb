...
unique_ptr<FunctionData> NSVBind(ClientContext &ctx, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> NSVInit(ClientContext &ctx, TableFunctionInitInput &input) {
...
void NSVScan(ClientContext &ctx, TableFunctionInput &input, DataChunk &output) {
	...
	idx_t count = std::min((idx_t)STANDARD_VECTOR_SIZE, bind.data->nrows - state.row);
	if (count == 0) {
...
static void LoadInternal(ExtensionLoader &loader) {
	TableFunction read_nsv_func("read_nsv", {LogicalType::VARCHAR}, NSVScan, NSVBind);
